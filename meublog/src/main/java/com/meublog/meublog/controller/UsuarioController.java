...
import com.meublog.meublog.service.UsuarioService;
...
public class UsuarioController {
	...
	public ResponseEntity<UsuarioLogin> Autentication(@RequestBody Optional<UsuarioLogin> userlogin){
		return 	userService.Logar(userlogin).map(resp->ResponseEntity.ok(resp))
				.orElse(ResponseEntity.status(HttpStatus.UNAUTHORIZED).build());
	}
	
	@PostMapping("/cadastrar")
	public ResponseEntity<UsuarioLogin> Post(@RequestBody UsuarioModel user){
		return 	ResponseEntity.status(HttpStatus.CREATED)
				.body(userService.CadastrarUser(user));
	}
}
